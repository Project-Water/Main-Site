...
function writeUserData() {
    ...
    if (confirm(`Are you sure you want to prefill data for ${school}? All previous tournament details will be overwritten`)) {
...
function presentEditorDialogue() {
    if (currentTeam != null) {
        var i;
        for (i = 0; i < data.length; i++) {
            if (data[i]['id'] == currentTeam)
        ...
        $("#teamSelect").val(data[i]['team']);
        $("#timeField").val(data[i]['time']);
        $("#locationSelect").val(data[i]['location']);
        $("#competitorSelect").val(data[i]['competitor']);
        $("#teamOther").val("");
        $("#locationOther").val("");
        $("#competitorOther").val("");
